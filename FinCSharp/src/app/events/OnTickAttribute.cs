...
namespace fin.app.events {
  ...
  public interface IForOnTickMethod {
    void ForOnTickMethod<TEvent>(SafeType<TEvent> eventType, Action<TEvent> handler) where TEvent : IEvent;
  }
  ...
  public class OnTickAttribute : Attribute {
    ...
    public static void SniffAndAddMethods(object onTickHandlerOwner, IForOnTickMethod target) {
      var forOnTickMethod = target.GetType().GetMethods().Where(m => m.Name == "ForOnTickMethod").Single();
      ...
      foreach (var onTickHandler in onTickHandlers) {
        ...
        var specificHandler = onTickHandler.CreateDelegate(actionType, onTickHandlerOwner);
        // TODO: Possible to remove this?
        Action<dynamic> genericHandler = evt => specificHandler.DynamicInvoke(evt);

        //var actionType = typeof(Action<>).MakeGenericType(eventParameterType);
        //Action<dynamic> handler = evt => onTickHandler.CreateDelegate(actionType, this).DynamicInvoke(new[] { evt });
        forOnTickMethod.MakeGenericMethod(eventParameterType).Invoke(target, new object[] { safeEventParameterType, genericHandler });
