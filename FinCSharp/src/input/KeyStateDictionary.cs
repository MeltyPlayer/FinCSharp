...
namespace fin.input {
  ...
  public class KeyStateDictionary : IKeyStateDictionary {
    private readonly IDictionary<KeyId, ButtonState> keyIdToState_ =
      new Dictionary<KeyId, ButtonState>();

    private readonly ISet<KeyId> instantaneousKeyIds_ = new HashSet<KeyId>();

    private readonly IDictionary<KeyId, ButtonState> keyIdToTransitionalState_ =
      new Dictionary<KeyId, ButtonState>();

    public ButtonState this[KeyId keyId] =>
      this.keyIdToState_.TryGetValue(keyId, out ButtonState state)
        ? state
        : ButtonState.UP;

    public void OnKeyDown(KeyId keyId) =>
      this.keyIdToTransitionalState_.Add(keyId, ButtonState.DOWN);

    public void OnKeyUp(KeyId keyId) =>
      this.keyIdToTransitionalState_.Add(keyId, ButtonState.UP);
    ...
    public void HandleTransitions() {
      ...
      foreach (var instanteousKeyId in this.instantaneousKeyIds_) {
        ...
        var newState = instantaneousState == ButtonState.PRESSED
          ? ButtonState.DOWN
          : ButtonState.UP;
      ...
      foreach (var keyIdAndTransitionalState in this.keyIdToTransitionalState_
        ...
        var newState = (transition == ButtonState.UP)
          ? (oldState == ButtonState.PRESSED || oldState == ButtonState.DOWN)
            ?
            ButtonState.RELEASED
            : ButtonState.DOWN
          : (oldState == ButtonState.RELEASED || oldState == ButtonState.UP)
            ? ButtonState.PRESSED
            : ButtonState.UP;
